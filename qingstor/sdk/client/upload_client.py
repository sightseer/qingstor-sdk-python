...
class UploadClient:
    ...
    def upload_large_file(self,file_name,file_path):
        ...
        if output.status_code==status_code.status_ok:
        ...
        while len(file_chunk.part_wait_list):
            ...
            if output.status_code==status_code.status_part_created:
                part_uploaded_list+=[{"part_number":part_index}]
                file_chunk.part_wait_list.pop(0)
                self.logger.info("%d Part Uploaded!"%part_index)
            elif output.status_code==status_code.status_bad_request:
                self.logger.error("Part Uploading Error!Error Cases:\n\
